...
class StudyInformationWidget(QWidget):
    ...
    def set_data(self, data: dict[str, object]) -> None:
        """Populate controls from a saved project."""
        ...
        self.study_name.setText(str(data.get("study_name", "")))
        self.drug_name.setText(str(data.get("drug_name", "")))
        self.subject_id.setText(str(data.get("subject_id", "")))
        self.dose.setText(
            "" if data.get("dose") is None else str(data.get("dose"))
        )
        self.body_weight.setText(
            ""
            if data.get("body_weight") is None
            else str(data.get("body_weight"))
        )
        self._set_combo_data(
            self.concentration_unit,
            str(data.get("concentration_unit", "ng/mL")),
        )
        self._set_combo_data(
            self.time_unit,
            str(data.get("time_unit", "h")),
        )
        self._set_combo_data(
            self.route,
            str(data.get("route", "Oral")),
        )
        self._set_combo_data(
            self.auc_method,
            str(data.get("auc_method", "linear_up_log_down")),
        )
        self.comments.setPlainText(str(data.get("comments", "")))
        self._previous_time_unit = str(self.time_unit.currentData())
        self._previous_concentration_unit = str(
            self.concentration_unit.currentData()
        )
        del blockers
        self.data_changed.emit()
